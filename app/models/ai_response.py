...
class StockNewsSummary(BaseModel):
    """Brief news summary for a single stock in portfolio context."""

    symbol: str
    sentiment: NewsSentiment
    article_count: int
    headline_summary: str = Field(description="One-line summary of key news")
    sentiment_driver: str = Field(description="Main factor driving sentiment")
    source_articles: List[SourceArticle] = Field(
        default_factory=list, description="Source articles with URLs"
    )
...
class PortfolioNewsAlert(BaseModel):
    """An alert or actionable item from portfolio news analysis."""

    urgency: str = Field(description="immediate, this_week, monitor")
    alert_type: str = Field(description="risk, opportunity, information")
    title: str
    description: str
...
class SectorTrend(BaseModel):
    """News trend for a sector represented in the portfolio."""

    sector: str
    sentiment: NewsSentiment
    key_development: str = Field(description="Main news development in this sector")
    affected_holdings: List[str] = Field(default_factory=list)


class PortfolioNewsAnalysis(BaseModel):
    """AI-generated news analysis across all portfolio holdings."""
    ...
    portfolio_sentiment: NewsSentiment = Field(description="Weighted sentiment across all holdings")
    sentiment_breakdown: str = Field(description="Breakdown of bullish/neutral/bearish holdings")
    confidence_level: str

    # Executive summary
    summary: str = Field(description="Portfolio-wide news synthesis")
    ...
    correlated_risks: List[str] = Field(
        default_factory=list, description="Risks affecting multiple holdings"
    )
    cross_portfolio_themes: List[str] = Field(
        default_factory=list, description="Themes appearing across multiple stocks"
    )

    # Actionable items
    alerts: List[PortfolioNewsAlert] = Field(default_factory=list)
    opportunities: List[str] = Field(default_factory=list)

    # Overall assessment
    portfolio_news_health: str = Field(description="Overall news environment assessment")
    key_takeaways: List[str] = Field(default_factory=list)

    disclaimer: str = Field(
        default="This is AI-generated analysis for educational purposes only. "
        "News sentiment can change rapidly and may not reflect future performance. Not financial advice.",
        description="Legal disclaimer",
    )
